...
mongoose
  .connect("mongodb://localhost:27017/Russian")
...
app.get("/quiz", async (req, res) => {
  try {
    ...
    const [quizzes, readings] = await Promise.all([
      quizzesPromise,
      readingPromise,
    ]);

    if (readings[0] && readings[0].questions.length > 6) {
      readings[0].questions = readings[0].questions
        .sort(() => 0.5 - Math.random())
        .slice(0, 6);
    }

    const mixedQuestions = [...quizzes, ...[readings[0]]].sort(
      () => 0.5 - Math.random()
    );
